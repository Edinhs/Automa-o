...
def maybe_finalize_automation(db: Session, task: AgentTask) -> None:
    """Delegate to the shared recalculate_automation_status service."""
    recalculate_automation_status(db, task_automation_id(task))
...
def update_files_from_result(db: Session, task: AgentTask, data: dict) -> None:
    ...
    for item in files:
        ...
        # Em reenvios de PDF, file_name e o nome do .pdf; original_file_name preserva o nome
        # pre-conversao para casar com WorkspaceFile.file_name quando nao houver file_id.
        ...
        if not db_file and original_file_name:
            db_file = db.query(WorkspaceFile).filter(WorkspaceFile.file_name == original_file_name, WorkspaceFile.is_deleted == False).first()
        if not db_file:
            continue
        file_status = item.get("status")
        playground_status = item.get("playground_status")
        if db_file.converted_to_pdf and file_status == "uploaded":
            file_status = "ready"
            playground_status = "Ready"
            db_file.ready_at = db_file.ready_at or now
...
def create_task(data: dict, authorization: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    if task_type == "connect_playground_session" and created_by_id:
        user = db.query(User).filter(User.id == created_by_id, User.is_deleted == False).first()
        if user:
            user.playground_connected = False
            user.playground_session_path = None
            user.playground_connected_at = None
            db.commit()
...
def complete_upload_batch(id: int, data: dict, db: Session = Depends(get_db)):
    ...
    for incoming in uploaded_files:
        ...
        if db_file.converted_to_pdf:
            db_file.status = "ready"
            db_file.playground_status = "Ready"
            db_file.ready_at = db_file.ready_at or now
            status_val = "ready"
            play_status_val = "Ready"
        else:
            db_file.status = "uploaded"
            db_file.playground_status = "Pending"
            status_val = "uploaded"
            play_status_val = "Pending"
        db_file.uploaded_at = db_file.uploaded_at or now
        db_file.updated_at = now
        canonical_files.append(
            {
                **source_item,
                ...
                "status": status_val,
                "playground_status": play_status_val,
            }
        )
...
def complete_task(id: int, data: Optional[dict] = None, db: Session = Depends(get_db)):
    ...
    if task.task_type == "upload_files_to_workspace":
        payload = parse_payload(task.payload_json)
        if payload.get("start_monitoring_after_upload", True) is not False and not payload.get("monitor_only"):
            # Se for Execução Completa, refaz o monitoramento de todos os arquivos ativos da automação
            full_execution = bool(payload.get("full_execution"))
            if full_execution and automation_id:
                files = db.query(WorkspaceFile).filter(
                    WorkspaceFile.automation_id == automation_id,
                    WorkspaceFile.is_deleted == False,
                ).all()
            else:
                # Caso contrário, pega apenas os arquivos desta tarefa de upload
                files = db.query(WorkspaceFile).filter(
...
def fail_task(id: int, data: dict, db: Session = Depends(get_db)):
    ...
    # Se a tarefa de conexao falhar, ou se qualquer outra tarefa falhar por erro de login,
    # redefinimos playground_connected do usuario para False no banco.
    err_msg = str(task.error_message or "").lower()
    is_login_error = "playgroundlogintimeout" in err_msg or "timeout aguardando login" in err_msg or "login manual necessario" in err_msg
    if task.task_type == "connect_playground_session" or is_login_error:
        user_id = task.created_by_id
        user = db.query(User).filter(User.id == user_id, User.is_deleted == False).first() if user_id else None
        if not user:
            user = db.query(User).filter(User.is_deleted == False).order_by(User.id.asc()).first()
