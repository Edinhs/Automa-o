...
# Well-known junk/temp files that must never enter an upload batch, even when they
# carry an allowed extension (e.g. Microsoft Office lock/owner files like
# "~$document.docx"). Matched by filename only so the rule is cross-platform safe.
IGNORED_FILENAME_PREFIXES = ("~$", "~", ".")
IGNORED_FILENAMES = {"thumbs.db", "desktop.ini"}


def is_ignored_junk_file(name: str) -> bool:
    """Return True if ``name`` is a well-known junk/temp/hidden file to skip.

    Conservative by design: only matches Microsoft Office lock/owner files
    (``~$``), other editor/OS temp & hidden files (``~`` / ``.`` prefixes) and
    Windows shell junk (``Thumbs.db`` / ``desktop.ini``, case-insensitive).
    Never matches a real document.
    """
    if not name:
        return False
    if name.startswith(IGNORED_FILENAME_PREFIXES):
        return True
...
def scan_monitored_folder(
    ...
    while stack:
        ...
        for entry in current_entries:
            ...
            if is_ignored_junk_file(path.name):
                junk_files_skipped += 1
                if junk_logs < MAX_IGNORED_FILE_LOGS:
                    junk_logs += 1
                    log_event(
                        log,
                        "info",
                        f"Arquivo de lixo/temporario ignorado: {path.name}",
                        metadata={"path": str(path), "reason": "junk_file"},
                    )
                continue
    ...
    stats = {
        ...
        "junk_files_skipped": junk_files_skipped,
