r"""Rola a grade da lista do SharePoint para a direita e tira print para revelar
colunas adicionais (ex.: Status) que ficaram fora da area visivel."""
...
def main():
    ...
    try:
        ...
        time.sleep(5)
        # Tenta rolar a grade horizontalmente
        page.mouse.wheel(1500, 0)
        time.sleep(2)
        out = BACKEND_DIR / "data" / "screenshots" / "sharepoint_list_scrolled.png"
        page.screenshot(path=str(out), full_page=False)
        print("Screenshot:", out)

        # Tenta abrir o formulario "Novo" para ver todos os campos em painel lateral
        try:
            page.get_by_role("button", name="Novo").click(timeout=8000)
            time.sleep(3)
