...
### Microsoft Graph (Outlook/Teams) — opcional, desligado por padrao

Necessario apenas para envio de e-mail/Teams (inclusive envio de relatorios). Com qualquer campo obrigatorio vazio, as integracoes respondem `not_configured` sem quebrar.

- `MS_GRAPH_TENANT_ID=` `MS_GRAPH_CLIENT_ID=` `MS_GRAPH_CLIENT_SECRET=` `MS_GRAPH_SENDER_USER=` — obrigatorios (app-only, MSAL client-credentials).
- `MS_GRAPH_TEAMS_WEBHOOK_URL=` (webhook) **ou** `MS_GRAPH_TEAMS_TEAM_ID=` + `MS_GRAPH_TEAMS_CHANNEL_ID=` (canal via Graph) — para mensagens no Teams.
- Caveats: anexo inline ~3 MB; post em canal app-only e *protected API* do Graph (pode dar 403 — webhook/e-mail sao mais confiaveis); o link de download do Teams usa o host da requisicao (local).

### Entrega por pasta (Power Automate) — alternativa sem App Registration

Quando o MS Graph nao pode ser usado (sem registro de app no Entra ID), o HUB pode **copiar cada relatorio gerado** (+ um `.json` sidecar com metadados) para uma pasta sincronizada ao OneDrive/SharePoint, e um fluxo do **Power Automate** entrega (Teams/e-mail). Passo a passo em `GUIA_POWER_AUTOMATE.md`.

- `REPORT_DELIVERY_PATH=` (+ `DEVELOPER_REPORT_DELIVERY_PATH=` / `OPERATIONAL_REPORT_DELIVERY_PATH=`) — vazio = desligado (geracao do relatorio nao e afetada). Aponte para uma pasta do OneDrive/SharePoint sincronizada.
- Disparo automatico no `persist_report` (ao gerar) + disparo manual `POST /api/integrations/reports/{id}/deliver-folder` (body opcional `{teams_channel, email_to, subject}`), rastreado em `/api/integrations/deliveries` (provider `PowerAutomate`).
- Atalho manual no dashboard (sem backend): o botao **Teams** de cada relatorio baixa o arquivo e abre o app do Teams no `teamsDeepLink` configurado em Configuracoes (anexo manual).
...
- `/api/reports`
- `/api/schedules`
- `/api/integrations` — status/teste, entregas e envio Teams/Outlook
  - `GET /api/integrations/graph/status` | `POST /api/integrations/graph/test`
  - `GET /api/integrations/deliveries` — historico de entregas (`pending|sent|failed|not_configured`)
  - `POST /api/integrations/reports/{id}/email` — envia o relatorio como anexo (Graph sendMail). Body: `{to_recipients, subject?, body?}`
  - `POST /api/integrations/reports/{id}/teams` — posta resumo + link do relatorio no canal/webhook
  - `POST /api/integrations/reports/{id}/deliver-folder` — copia o relatorio (+sidecar) para `REPORT_DELIVERY_PATH` (pickup do Power Automate)
- `/api/executions`
