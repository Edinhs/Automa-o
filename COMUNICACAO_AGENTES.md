...
| `T-013` | **Usuário (Ederson)** | **Automações Personalizadas / IPC Workspace Updater** (feature nova, em andamento) | `Em Progresso` | `routers/custom_automations.py`, `custom_automations/ipc_workspace_updater/*`, `main.py`, `dist/assets/custom_automation.js`, `run_ipc_updater.bat` | Router `/api/custom-automations` + updater IPC (parser de documentos, db_helper, cr_processor) + launcher. Commitado junto a pedido (WIP não revisado pelo Claude). |
